...
def test_easy_ensemble_classifier_recalibrate_error():
    ...
    with pytest.raises(ValueError, match=err_msg):
        EasyEnsembleClassifier(recalibrate=True).fit(X, y)


def test_easy_ensemble_classifier_recalibrate():
    ...
    X_train, X_test, y_train, y_test = train_test_split(X, y, random_state=0)
    ee_uncalibrated = EasyEnsembleClassifier(
        n_estimators=50,
    ...
    ee_recalibrated = EasyEnsembleClassifier(
        n_estimators=50,
        estimator=DecisionTreeClassifier(),
        random_state=42,
        n_jobs=-1,
    ...
    log_loss_uncalibrated = log_loss(y_test, ee_uncalibrated.predict_proba(X_test))
    ...
    log_loss_uncalibrated = log_loss(
        y_test, np.exp(ee_uncalibrated.predict_log_proba(X_test))
    )
    log_loss_recalibrated = log_loss(
        y_test, np.exp(ee_recalibrated.predict_log_proba(X_test))
    )
    assert log_loss_uncalibrated > log_loss_recalibrated

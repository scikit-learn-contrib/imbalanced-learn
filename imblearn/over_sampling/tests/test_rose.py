...
def test_rose():
    ...
    print("covs: ", cov1, "\n-\n\n",cov2, "\n-\n\n",cov3, "\n-\n\n")
    # generate data blobs
    cl1 = np.array(np.random.multivariate_normal([1,1,1],cov = cov1,size =ns[0]))
    cl2 = np.array(np.random.multivariate_normal([7,7,7],cov = cov2,size =ns[1]))
    cl3 = np.array(np.random.multivariate_normal([2,9,9],cov = cov3,size =ns[2]))
    
    # assemble dataset
    X = np.vstack((cl1,cl2,cl3))
    y = np.hstack((np.array([1]*ns[0]),np.array([2]*ns[1]),np.array([3]*ns[2])))

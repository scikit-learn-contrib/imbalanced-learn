...
def test_nearmiss_init():
    ...
    ratio = 1.
    verbose = True
    nm3 = NearMiss(ratio=ratio, random_state=RND_SEED, verbose=verbose,
...
def test_nm3_fit():
    ...
    assert_equal(nm3.maj_c_, 1)
    assert_equal(nm3.stats_c_[0], 500)
...
def test_nm3_fit_sample_auto():
    ...
    y_gt = np.load(os.path.join(currdir, 'data', 'nm3_y.npy'))
...
def test_nm3_fit_sample_auto_indices():
    ...
    X_gt = np.load(os.path.join(currdir, 'data', 'nm3_x.npy'))
    y_gt = np.load(os.path.join(currdir, 'data', 'nm3_y.npy'))
    idx_gt = np.load(os.path.join(currdir, 'data', 'nm3_idx.npy'))
    assert_array_equal(X_resampled, X_gt)
    assert_array_equal(y_resampled, y_gt)
...
def test_nm3_fit_sample_half():
    ...
    y_gt = np.load(os.path.join(currdir, 'data', 'nm3_y_05.npy'))
    assert_array_equal(X_resampled, X_gt)
    assert_array_equal(y_resampled, y_gt)

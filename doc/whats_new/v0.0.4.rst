...
- Replace the parameter ``ratio`` by ``sampling_target``. :issue:`411` by
  :user:`Guillaume Lemaitre <glemaitre>`.

- Enable to use a ``float`` with binary classification for
  ``sampling_target``. :issue:`411` by :user:`Guillaume Lemaitre <glemaitre>`.
...
- Deprecate ``ratio`` in favor of ``sampling_target``. :issue:`411` by
  :user:`Guillaume Lemaitre <glemaitre>`.
